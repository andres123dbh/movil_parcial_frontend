    ...
    if (token != null) {
      try {
        http.Response response =
            await http.get(Uri.parse(url), headers: {"accessToken": token});
                ...
                children: [
                  if (favorite.contains(productList[i]['_id'])) ...[
                    ProductCardAlt(
                        ...
                        double.parse("${productList[i]['rating']}"),
                        "${productList[i]['image']}",
                        true)
                  ] else ...[
                    ProductCardAlt(
                  ...
                  if (favorite.contains(productList[i + 1]['_id'])) ...[
                    ProductCardAlt(
                        "${productList[i + 1]['_id']}",
                        "${productList[i + 1]['title']}",
                        "${productList[i + 1]['seller']}",
                        double.parse("${productList[i + 1]['rating']}"),
                        "${productList[i + 1]['image']}",
                        true)
                  ] else ...[
                    ProductCardAlt(
                        "${productList[i + 1]['_id']}",
